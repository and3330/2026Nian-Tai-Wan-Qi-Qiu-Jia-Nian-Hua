...
- [x] **#12** 倒數 + 即時剩餘票數動畫 — `EventCountdown` (天/時/分/秒, pop animation) + `AnimatedNumber` (rAF count-up + flash). HomePage hero 加倒數、即時剩餘名額用動畫數字 + 30s polling；CarnivalPage 日期卡片剩餘數同樣動畫
- [ ] **#13** 一鍵分享社群
...
- **Stripe webhook** (`/api/payments/stripe/webhook`) mounted BEFORE `express.json` with raw body parser; `checkout.session.completed` marks paid + auto-issues invoice. Best-effort `confirmStripePayment` from result page covers webhook delays. (Path MUST match the URL configured in the Stripe dashboard and the `/api/payments/*` convention — a mismatch returns 404 and silently leaves orders unconfirmed.)
